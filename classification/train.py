...
def main(args):
    ...
    for epoch in range(args.epochs):
        # train
        # train_loss, train_acc, train_kappa1, train_truee, train_predd = (0, 0, 0, [], [])
        ...
        # validate
        # val_loss, val_acc, val_kappa1, val_truee, val_predd = (0, 0, 0, [], [])
        val_loss, val_acc, val_kappa1, val_truee, val_predd = evaluate(model=model,
                                                                       data_loader=val_loader,
                                                                       device=device,
                                                                       epoch=epoch)
        print("val_kappa= %.4f" % val_kappa1)
        with open(join("log.json"), "w") as f:
            f.write(f'{{"epoch": {epoch}, "train_loss": {train_loss:.4f}, "train_acc": {train_acc:.4f}, "train_kappa": {train_kappa1:.4f}, "val_loss": {val_loss:.4f}, "val_acc": {val_acc:.4f}, "val_kappa": {val_kappa1:.4f}, "time": "{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}"}}\n')
        ...
        if best_acc < val_acc:
            PATH = "./data/8422229/BMP_classification/BMP_classification/train"
            ...
            val_cfmfig.savefig(join(PATH, "cancer_resnet50.png"))
